...
def eval_dataset(args):
    ...
    if args.task == 'all':
        ...
        if args.num_chunks == 8:
            balance_w = [1, 1, 1.3, 0.5, 0.7, 1, 1.5, 1]  # balance for gpu time
    else:
        tasks = [args.task]
    ...
    for task in tasks:
        out[task] = []
        task_samples = json.load(open(f'benchmarks/mlvu/json/{task}.json'))
    ...
    for sample in tqdm(samples):
        v = sample['video']
        task = sample['task']
        video_path = f'{args.video_folder}/{task}/{v}'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-folder", type=str, default="outputs/mlvu")
