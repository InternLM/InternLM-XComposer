...
class InternLM2ForCausalLM(InternLM2PreTrainedModel):
    ...
    def input_formulate(self, img_embeds, img_split, image_num, texts, temp_max_len=None, use_mask=True):
        temp_embeds = []
        temp_im_mask = []
        temp_tars = []
    ...
    def prompt_warp(self, images, text):
        ...
        if pre_pos > 0:
            ...
            text[0] = text[0].replace('[UNUSED_TOKEN_146]user\n', '[UNUSED_TOKEN_146]user\n<IM_POS>', 1)
    ...
    def forward(
        ...
        **kwargs
        ...
        if samples:
            ...
            for im in image:
                if samples['data_type'][0] == 'video_mem':
                    image_num.append(1)
            ...
            if samples['data_type'][0] != 'nlp':
                ...
                if samples['data_type'][0] == 'video_mem':
                    ...
                    for img in image:
                        ...
                        if type(img) is list:
                            if len(img) == 1:
                                ...
                                print('Video part number error', len(img))

                            _, b, n, c = glb.shape
                            glb = self.video_mem_proj(glb.view(1, -1, c))
                            glb_text, _ = self.text2emb('This is video overview memory:', add_special_tokens=False, cal_loss=False)
                            temp_emb.append(glb_text)
                            temp_emb.append(glb)
                            
                            if lol is not None:
                                _, b, n, c = lol.shape
                                lol = self.video_mem_proj(lol.view(1, -1, c))
                                lol_text, _ = self.text2emb('This is question related video memory:', add_special_tokens=False, cal_loss=False)
                                temp_emb.append(lol_text)
                                temp_emb.append(lol)

                            if im is not None:
                                im_embeds, _ = self.img2emb([im])
                                im_text, _ = self.text2emb('This is question related image:', add_special_tokens=False, cal_loss=False)
                                temp_emb.append(im_text)
                                temp_emb.append(im_embeds)

                            temp_emb = torch.cat(temp_emb, dim=1)
                            img_split.append(temp_emb.shape[1])
                            img_embeds.append(temp_emb)
                    ...
                    use_mask = True
                inputs_embeds, attention_mask, targets, im_mask = self.input_formulate(img_embeds, img_split, image_num, text, use_mask=use_mask)

            else:
                img_embeds, img_split = self.img2emb([torch.zeros(1,3,560,560).to(self.device).to(self.model.dtype)])
                glb_embeds = self.video_mem_proj(torch.zeros(1,16,1536).to(self.device).to(self.model.dtype))
                inputs_embeds, attention_mask, targets, im_mask = self.input_formulate([], [], [0] * len(text), text)
                inputs_embeds += img_embeds.sum() * 0
                inputs_embeds += glb_embeds.sum() * 0
