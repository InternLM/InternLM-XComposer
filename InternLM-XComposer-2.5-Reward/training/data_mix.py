...
def conv2text(sources, end=True):
    END_TOKEN = '[UNUSED_TOKEN_145]\n'
    conversation = ''

    for idx, sentence in enumerate(sources):
        from_str = sentence['from']
        value_str = sentence['value'].strip()

        if from_str.lower() == 'system':
            from_str = '[UNUSED_TOKEN_146]system\n'
            temp = from_str + value_str + END_TOKEN
        elif from_str.lower() == 'human' or from_str.lower() == 'user':
    ...
    if end:
        return conversation + '</s>[UNUSED_TOKEN_130]'
    else:
        return conversation
...
class Mix_dataset(Dataset):
    ...
    def __init__(self,
        ...
        self.data_num = np.sum(self.data_num_multi) + np.sum(self.data_num_text)
        self.use_multi = 0
    ...
    def __getitem__(self, index):
        ...
        if len(self.datasets_multi) > 0 and (self.use_multi <= 1
...
class Sample_dataset(Dataset):
    ...
    def __get_item__(self, i):
        conversations_a = self.raw_data[i]['conversations_a']
        conversations_b = self.raw_data[i]['conversations_b']

        chosen = conv2text(conversations_a)
        rejected = conv2text(conversations_b)

        sample = dict(chosen=chosen, rejected=rejected)
        if self.has_img:
